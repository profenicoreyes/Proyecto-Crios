/* CRIOS Studio - geometry area migration catalog */
(function(){
  'use strict';

  var runtime=window.CRIOS_RUNTIME_MISSION_HANDLERS;
  if(!runtime||typeof runtime.createPublishedMissionSpec!=='function')throw new Error('Runtime mission handlers are required.');
  ...
  function metadata(numberValue,title,shortTitle,mapSubtitle,styleToken,location,objective,difficulty,duration,tags){return{number:numberValue,title:title,shortTitle:shortTitle,map:{title:title,subtitle:mapSubtitle,styleToken:styleToken},classification:{subject:'matematica',topic:'geometria',subtopic:'calculoAreas',level:'educacion-media',difficulty:difficulty},narrative:{location:location,objective:objective},activity:{type:'respuesta-numerica-con-procedimiento',durationMinutes:duration,tags:tags.slice()}};}
  function input(missionId,payload){return{missionId:missionId,handlerId:'crios.geometry.declarative-area',handlerVersion:'1.0.0',payload:payload};}

  function energy(){return input('energy',{
  ...
  function greenhouse(){return input('greenhouse',{
  ...
  function hangar(){return input('hangar',{
  ...
  var factories=Object.freeze({energy:energy,greenhouse:greenhouse,ice:ice,hangar:hangar});
  var order=Object.freeze(['energy','greenhouse','ice','hangar']);
  function get(missionId){var id=String(missionId||'');if(!Object.prototype.hasOwnProperty.call(factories,id)){var error=new Error('Unknown missionId: '+id+'.');error.code='MISSION_SPEC_NOT_FOUND';throw error;}var created=runtime.createPublishedMissionSpec(factories[id]());if(!created.success){var invalid=new Error(created.error.message);invalid.code=created.error.code;throw invalid;}return created.spec;}
  function list(){return Object.freeze(order.map(function(id){return get(id);}));}
  window.CRIOS_STUDIO_GEOMETRY_AREA_SPEC_CATALOG=Object.freeze({version:'1.0.0',list:list,get:get});
})();