/* CRIOS MVP 1.0 - visual acceptance evidence */
(function(){
  'use strict';
  ...
  async function createControlledDocument(options) {
    ...
    frame.style.cssText = 'position:fixed;left:0;top:0;border:0;width:' + width + 'px;height:' + height + 'px;z-index:2147483647;background:#05080d;';
    frames.push(frame);
    document.body.appendChild(frame);
    frame.src = './mvp-visual-acceptance.test.html?shell=1&token=' + Math.random().toString(36).slice(2);
    await waitFor(function(){
      return frame.contentWindow && frame.contentWindow.location.pathname.endsWith('/tests/mvp-visual-acceptance.test.html');
    }, 20000, 'shell-ready');
  ...
  function contrastRatio(foreground, background) {
    if (!foreground || !background || foreground.a < 1 || background.a < 1) return null;
  ...
  function effectiveBackground(child, element) {
    var current = element;
    while (current && current.nodeType === 1) {
      var style = child.getComputedStyle(current);
      var parsed = parseRgb(style.backgroundColor);
      if (parsed && parsed.a >= 0.98) return parsed;
      current = current.parentElement;
    }
    return parseRgb(child.getComputedStyle(child.document.body).backgroundColor) || { r: 255, g: 255, b: 255, a: 1 };
  }

  function elementSnapshot(child, id) {
    var element = child.document.getElementById(id);
    if (!element) return { id: id, exists: false };
    var style = child.getComputedStyle(element);
    var rect = element.getBoundingClientRect();
    var foreground = parseRgb(style.color);
    var background = effectiveBackground(child, element);
    var visible = style.display !== 'none' && style.visibility !== 'hidden' && Number(style.opacity || 1) > 0.01 && rect.width > 0 && rect.height > 0;
    return {
      id: id,
      exists: true,
      visible: visible,
      disabled: Boolean(element.disabled),
      text: String(element.textContent || element.value || '').trim().slice(0, 180),
      rect: {
      ...
      inViewport: visible && rect.left >= -1 && rect.top >= -1 && rect.right <= child.innerWidth + 1 && rect.bottom <= child.innerHeight + 1,
  ...
  function visibleInteractiveSnapshot(child) {
    var selector = 'button,input,select,textarea,a[href],[role="button"]';
    return Array.prototype.map.call(child.document.querySelectorAll(selector), function(element){
      var style = child.getComputedStyle(element);
      var rect = element.getBoundingClientRect();
      if (style.display === 'none' || style.visibility === 'hidden' || Number(style.opacity || 1) <= 0.01 || rect.width <= 0 || rect.height <= 0) return null;
      return {
        id: element.id || null,
        tag: element.tagName.toLowerCase(),
        text: String(element.textContent || element.value || element.getAttribute('aria-label') || '').trim().slice(0, 120),
        disabled: Boolean(element.disabled),
        rect: {
          left: Number(rect.left.toFixed(2)),
          top: Number(rect.top.toFixed(2)),
        ...
        inViewport: rect.left >= -1 && rect.top >= -1 && rect.right <= child.innerWidth + 1 && rect.bottom <= child.innerHeight + 1
      };
    }).filter(Boolean);
  }

  function visualAudit(controlled, label, criticalIds) {
    var child = controlled.child;
    var doc = child.document;
    var root = doc.documentElement;
    var body = doc.body;
    var active = doc.querySelector('.screen.active');
    var critical = (criticalIds || []).map(function(id){ return elementSnapshot(child, id); });
    var missing = critical.filter(function(item){ return !item.exists; });
    var invisible = critical.filter(function(item){ return item.exists && !item.visible; });
    var outside = critical.filter(function(item){ return item.visible && !item.inViewport; });
    var viewport = {
      width: child.innerWidth,
      height: child.innerHeight,
      documentScrollWidth: Math.max(root ? root.scrollWidth : 0, body ? body.scrollWidth : 0),
      documentScrollHeight: Math.max(root ? root.scrollHeight : 0, body ? body.scrollHeight : 0)
    };
    var audit = freeze({
      label: label,
      activeScreen: active ? active.id : null,
      viewport: viewport,
      horizontalOverflowPx: Math.max(0, viewport.documentScrollWidth - viewport.width),
      critical: critical,
      missingCritical: missing.map(function(item){ return item.id; }),
      invisibleCritical: invisible.map(function(item){ return item.id; }),
      outsideCritical: outside.map(function(item){ return item.id; }),
      interactives: visibleInteractiveSnapshot(child)
    });
    visualCheckpoints.push(audit);
    equal('VISUAL_' + label.toUpperCase().replace(/[^A-Z0-9]+/g, '_') + '_MISSING', audit.missingCritical, []);
    equal('VISUAL_' + label.toUpperCase().replace(/[^A-Z0-9]+/g, '_') + '_INVISIBLE', audit.invisibleCritical, []);
    equal('VISUAL_' + label.toUpperCase().replace(/[^A-Z0-9]+/g, '_') + '_OUTSIDE', audit.outsideCritical, []);
    assert('VISUAL_' + label.toUpperCase().replace(/[^A-Z0-9]+/g, '_') + '_NO_HORIZONTAL_OVERFLOW', audit.horizontalOverflowPx <= 1, 'Overflow horizontal=' + audit.horizontalOverflowPx);
    return audit;
  }

  async function visualCheckpoint(controlled, label, criticalIds) {
    var audit = visualAudit(controlled, label, criticalIds);
    var id = ++checkpointCounter;
    window.CRIOS_MVP_VISUAL_ACK = null;
    window.CRIOS_MVP_VISUAL_CHECKPOINT = freeze({
      id: id,
      label: label,
    ...
    await waitFor(function(){ return window.CRIOS_MVP_VISUAL_ACK === id; }, 60000, 'visual-checkpoint-' + label);
    return audit;
  }
  ...
  async function solveMissionThroughUi(child, missionId, label, controlled, visualLabel) {
    ...
    if (controlled && visualLabel) {
      await visualCheckpoint(controlled, visualLabel, ['procedure-' + missionId, 'answer-' + missionId]);
    }
  ...
  async function run() {
    ...
    try {
      ...
      await visualCheckpoint(studioControlled, '01-studio-published', ['studioRuntimeLaunchLink']);
      ...
      first.go('reveal');
      await visualCheckpoint(firstRuntime, '02-identity-desktop', ['userNameInput','characterNameInput','groupInput','identifyButton']);
      ...
      await visualCheckpoint(firstRuntime, '03-entry-desktop', ['campaignEntryButton']);
      ...
      await visualCheckpoint(firstRuntime, '04-map-desktop-empty', ['card-energy','card-greenhouse','card-ice','card-hangar','finalBtn']);

      details.missionSolutions.push(await solveMissionThroughUi(first, 'energy', 'FIRST_HALF', firstRuntime, '05-mission-energy-desktop'));
      details.missionSolutions.push(await solveMissionThroughUi(first, 'greenhouse', 'FIRST_HALF', null, null));
      ...
      await visualCheckpoint(firstRuntime, '06-map-desktop-half', ['card-energy','card-greenhouse','card-ice','card-hangar','finalBtn']);
      ...
      details.missionSolutions.push(await solveMissionThroughUi(resumed, 'ice', 'SECOND_HALF', null, null));
      details.missionSolutions.push(await solveMissionThroughUi(resumed, 'hangar', 'SECOND_HALF', null, null));
      ...
      await visualCheckpoint(resumedRuntime, '07-map-desktop-complete', ['card-energy','card-greenhouse','card-ice','card-hangar','finalBtn','progressLabel']);
      ...
      await visualCheckpoint(resumedRuntime, '08-final-desktop', ['finalProcedure','finalAnswer']);
      ...
      await visualCheckpoint(resumedRuntime, '09-credits-desktop', ['creditsTotal']);
      await waitFor(function(){
      ...
      await visualCheckpoint(completedRuntime, '10-identity-narrow', ['userNameInput','characterNameInput','groupInput','identifyButton']);
      ...
      await visualCheckpoint(completedRuntime, '11-entry-narrow', ['campaignEntryButton']);
      ...
      await visualCheckpoint(completedRuntime, '12-map-narrow-complete', ['card-energy','card-greenhouse','card-ice','card-hangar','finalBtn','progressLabel']);
      ...
      await visualCheckpoint(completedRuntime, '13-identity-narrow-reset', ['userNameInput','characterNameInput','groupInput','identifyButton']);
      ...
      await visualCheckpoint(completedRuntime, '14-map-narrow-new-session', ['card-energy','card-greenhouse','card-ice','card-hangar','finalBtn','progressLabel']);
      ...
      var result = freeze({
        ...
        visualCheckpoints: visualCheckpoints.slice(),
        details: details,
        cleanup: {
      ...
      Object.defineProperty(window, 'CRIOS_MVP_VISUAL_ACCEPTANCE_RESULTS', {
        value: result,
        enumerable: true,
        configurable: false,
        writable: false
