(function(){
  ...
  Promise.resolve().then(function(){
    ...
    equal('versión exacta',api.version,'1.1.0');
    ...
    equal('legacy default sin publicationId',legacyDefault.publicationId,null);
    ...
    equal('published configurado sin campaignId explícito',publishedDefault.campaignId,null);
    equal('published configurado sin publicationId explícito',publishedDefault.publicationId,null);
    ...
    equal('legacy explícito sin campaignId',explicitLegacy.campaignId,null);
    equal('legacy explícito sin publicationId',explicitLegacy.publicationId,null);

    var explicitPublished=selection('legacy','?source=published&campaignId=campaign-alpha&publicationId=publication-alpha');
    ...
    equal('published conserva publicationId',explicitPublished.publicationId,'publication-alpha');
    equal('published explícito no bloqueado',explicitPublished.blocked,false);

    var missingCampaign=selection('legacy','?source=published&publicationId=publication-alpha');
    ...
    var missingPublication=selection('legacy','?source=published&campaignId=campaign-alpha');
    assert('published sin publicationId bloqueado',api.isRuntimeLaunchSelection(missingPublication));
    equal('published sin publicationId código',missingPublication.error.code,'PUBLICATION_ID_REQUIRED');

    var sourceRequired=selection('legacy','?campaignId=campaign-alpha&publicationId=publication-alpha');
    ...
    var explicitOverridesInvalid=selection('other','?source=published&campaignId=campaign-beta&publicationId=publication-beta');
    ...
    var missingContractKnown=selection('legacy','?source=published&campaignId=campaign-gamma&publicationId=publication-gamma',null);
    ...
    equal('builder published interoperable',window.CRIOS_RUNTIME_LAUNCH.buildPublishedLaunchSearch('campaign alpha','publication alpha'),'?source=published&campaignId=campaign%20alpha&publicationId=publication%20alpha');
