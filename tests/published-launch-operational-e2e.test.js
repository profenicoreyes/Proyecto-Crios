...
(function(){
  ...
  async function run() {
    ...
    try {
      ...
      resumedRuntime = await launchRuntime(new URL(launchHref, studio.location.href).href, 'Runtime published resume');
      var resumed = resumedRuntime.child;
      await waitGroups(resumed, 'runtime-resume');
      resumed.go('reveal');
      assert('PUBLISHED_RESUME_LOGIN_VISIBLE', !resumed.document.getElementById('missionLogin').classList.contains('hidden'), 'La recarga published omitió la confirmación de identidad.');
      assert('PUBLISHED_RESUME_WELCOME_HIDDEN', resumed.document.getElementById('missionWelcome').classList.contains('hidden'), 'La recarga published mostró una campaña aún no preparada.');
      assert('PUBLISHED_RESUME_REAL_NAME_PREFILLED', resumed.document.getElementById('userNameInput').value === 'Alumno Stage 4', 'Nombre real no recuperado.');
      assert('PUBLISHED_RESUME_CHARACTER_PREFILLED', resumed.document.getElementById('characterNameInput').value === 'Operador Stage 4', 'Personaje no recuperado.');
      assert('PUBLISHED_RESUME_GROUP_PREFILLED', resumed.document.getElementById('groupInput').value === 'ALUMNO_STAGE4', 'Grupo no recuperado.');
      await resumed.identifyUser();
      await waitFor(function(){ return !resumed.document.getElementById('missionWelcome').classList.contains('hidden'); }, 30000, 'runtime-resumed');
      var resumedSession = getSession(resumed);
      assert('PUBLISHED_RESUME_SESSION_PRESENT', Boolean(resumedSession), 'Sesión recuperada ausente.');
      assert('PUBLISHED_RESUME_SESSION_ID_STABLE', resumedSession.idSesion === solvedSession.idSesion, 'La recarga creó otra sesión.');
      assert('PUBLISHED_RESUME_CAMPAIGN_STABLE', resumedSession.campana.campaignId === campaignId, 'Campaña recuperada incorrecta.');
      assert('PUBLISHED_RESUME_PUBLICATION_STABLE', resumedSession.campana.publicationId === publicationId, 'Publicación recuperada incorrecta.');
      equal('PUBLISHED_RESUME_MISSION_COUNT', resumed.CRIOS.obtenerMisionesActivas().length, 4);
      assert('PUBLISHED_RESUME_SOLVED_MISSION_PRESERVED', resumedSession.misiones[solved.missionId].answerCorrect === true, 'La misión resuelta no se preservó.');
      assert('PUBLISHED_RESUME_ENTRY_BUTTON', resumed.document.getElementById('campaignEntryButton').textContent.trim() === 'Continuar campaña', 'Entrada de campaña recuperada incorrecta.');
      var resumeDiag = snapshotDiagnostics('runtime-resume', resumed);
      equal('PUBLISHED_RESUME_PAGE_ERRORS', resumeDiag.pageErrors.length, 0);
      equal('PUBLISHED_RESUME_CONSOLE_ERRORS', resumeDiag.consoleErrors.length, 0);
      equal('PUBLISHED_RESUME_REJECTIONS', resumeDiag.unhandledRejections.length, 0);
      resumedRuntime.frame.remove();
      resumedRuntime = null;

      staleLegacyRuntime = await launchRuntime(new URL('../index.html?source=legacy', studio.location.href).href, 'Runtime legacy with stale published session');
      var staleLegacy = staleLegacyRuntime.child;
      await waitGroups(staleLegacy, 'runtime-legacy-stale-session');
      staleLegacy.go('reveal');
      assert('LEGACY_STALE_LOGIN_VISIBLE', !staleLegacy.document.getElementById('missionLogin').classList.contains('hidden'), 'Legacy ocultó el formulario con una sesión published previa.');
      assert('LEGACY_STALE_WELCOME_HIDDEN', staleLegacy.document.getElementById('missionWelcome').classList.contains('hidden'), 'Legacy mostró la sesión published previa.');
      var staleSessionBefore = getSession(staleLegacy);
      assert('LEGACY_STALE_PUBLISHED_SESSION_PRESENT', staleSessionBefore && staleSessionBefore.idSesion === solvedSession.idSesion, 'No se conservó la sesión published para comprobar su reemplazo.');
      await identify(staleLegacy, 'Alumno Stage 4', 'Operador Stage 4', 'ALUMNO_STAGE4');
      await waitFor(function(){ return !staleLegacy.document.getElementById('missionWelcome').classList.contains('hidden'); }, 30000, 'runtime-legacy-stale-identified');
      var staleLegacySession = getSession(staleLegacy);
      assert('LEGACY_STALE_NEW_SESSION', staleLegacySession && staleLegacySession.idSesion !== solvedSession.idSesion, 'Legacy reutilizó la sesión published.');
      assert('LEGACY_STALE_SESSION_NOT_PUBLISHED', staleLegacySession && staleLegacySession.campana && staleLegacySession.campana.sourceMode !== 'published', 'Legacy conservó identidad published.');
      equal('LEGACY_STALE_MISSION_COUNT', staleLegacy.CRIOS.obtenerMisionesActivas().length, 4);
      var staleLegacyDiag = snapshotDiagnostics('runtime-legacy-stale-session', staleLegacy);
      equal('LEGACY_STALE_PAGE_ERRORS', staleLegacyDiag.pageErrors.length, 0);
      equal('LEGACY_STALE_CONSOLE_ERRORS', staleLegacyDiag.consoleErrors.length, 0);
      equal('LEGACY_STALE_REJECTIONS', staleLegacyDiag.unhandledRejections.length, 0);
      staleLegacyRuntime.frame.remove();
      staleLegacyRuntime = null;
